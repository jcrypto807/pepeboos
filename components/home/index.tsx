...
export default function Home({priceData} : {priceData: any}) {
  ...
  const [ionxValue, setIonxValue] = useState(
    Math.round(priceData[coin.name] / priceData["IONX"]).toString()
  );
        ...
        toast.success("Successful!", {
          description: (
            <div className="text-black">
              {value} {coin.name} has been deposited for buying ${ionxValue} $PEPEBOSS.{" "}
              <a
                href={`${coin.explorer}tx/${_data.hash}`}
                target="_blank"
              >
                See transaction
              </a>
              <br />
              All the purchased tokens will be distributed to your wallet address once the presale is over.
            </div>
          ),
          duration: 5000,
    ...
    onSuccess: async(_data) => {
      if (_data.success) {
        toast.success("Successful!", {
          description: (
            <div className="text-black">
              {value} {coin.name} has been deposited for buying ${ionxValue} IONX.{" "}
              <a
                href={`${coin.explorer}tx/${_data.hash}`}
                target="_blank"
              >
  ...
  const handleBuy = () => {
    if (coin.name === "SOL") {
      ...
      if (coin.address === zeroAddress)
        buyEvmCoinMutation.mutate({coin, amount: BigInt(Number(value) * 10 ** coin.decimals)})
      else
        buyEvmTokenMutation.mutate({coin, amount: BigInt(Number(value) * 10 ** coin.decimals)})
    }
  }
  ...
  setIonxValue(
      Math.round(
        (parseFloat(value) * priceData[coin.name]) / priceData["IONX"]
      ).toString()
    );
  }, [coin])
  ...
  return (
    <main className="flex flex-col gap-[16px] max-w-[450px] p-4 row-start-2 bg-orange-100 items-center border border-amber-400 rounded-2xl">
          ...
          <div className="w-full space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center space-x-2">
                  <Avatar className="h-8 w-8 rounded-full">
                    <AvatarImage src="/pepe%20boss%20.png" alt="PEPEBOSS" />
                    <AvatarFallback className="rounded-full">
                      CN
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex flex-col">
                    <span className="text-md font-medium">$PEPEBOSS</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      >
        ...
        <div className="">({address?.slice(0,6)}...{address?.slice(-6)})</div>
      </Button> : <Button
        variant="outline"
        size="lg"
        onClick={() => open()}
